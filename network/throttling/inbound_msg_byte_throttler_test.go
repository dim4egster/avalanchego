...
func TestInboundMsgByteThrottlerCancelContext(t *testing.T) {
	...
	throttler.lock.Lock()
	assert.Len(throttler.nodeToWaitingMsgID, 1)
	assert.Contains(throttler.nodeToWaitingMsgID, vdr2ID)
	...
	assert.NotContains(throttler.nodeToWaitingMsgID, vdr2ID)
}
...
func TestInboundMsgByteThrottler(t *testing.T) {
	...
	assert.Len(throttler.nodeToAtLargeBytesUsed, 1)
	assert.Len(throttler.nodeToWaitingMsgID, 0)
	...
	throttler.lock.Lock()
	assert.Len(throttler.nodeToWaitingMsgID, 1)
	assert.Contains(throttler.nodeToWaitingMsgID, vdr1ID)
	assert.EqualValues(1, throttler.waitingToAcquire.Len())
	...
	throttler.lock.Lock()
	assert.Len(throttler.nodeToWaitingMsgID, 2)
	...
	throttler.lock.Lock()
	assert.Len(throttler.nodeToWaitingMsgID, 3)
	assert.Contains(throttler.nodeToWaitingMsgID, nonVdrID)
	...
	assert.Len(throttler.nodeToVdrBytesUsed, 0)
	assert.EqualValues(0, throttler.remainingAtLargeBytes)
	assert.NotContains(throttler.nodeToWaitingMsgID, nonVdrID)
	assert.EqualValues(0, throttler.waitingToAcquire.Len())
