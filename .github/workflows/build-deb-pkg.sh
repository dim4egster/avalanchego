...
AVALANCHE_LIB_DIR=$DEBIAN_BASE_DIR/usr/local/lib/qmallgo
...
dpkg-deb --build debian qmallgo-$TAG-$ARCH.deb
aws s3 cp qmallgo-$TAG-$ARCH.deb s3://${BUCKET}/linux/debs/ubuntu/$RELEASE/$ARCH/
rm -rf $PKG_ROOT
