...
## Recap of Qmall Subnets

The Qmall Network is composed of multiple validator sets and blockchains. A validator set defines a group of validators and their specified weights in the consensus process. A subnet is a validator set working together to achieve consensus on a set of blockchains. Every blockchain is validated by one subnet, and one subnet can validate many blockchains.

There is a special subnet inherent to the Qmall Network called the Primary Network. The Primary Network is validated by every node on the Qmall network. All subnets' validator sets are required to be a subset of the Primary Network's validator set. That is, if a validator belongs to a subnet then it also belongs to the Primary Network. The Primary Network validates three blockchains that are inherent to the Qmall Network: the P-Chain, C-Chain, and X-Chain.
...
The consensus engine is general and agnostic to the application semantics of the blockchain. There are two consensus engine implementations in AvalancheGo: Snowman and Qmall. Snowman provides a consensus engine for linear chains and Qmall provides a consensus engine for DAGs. These consensus engine implementations can be re-used for multiple different blockchains in the Qmall ecosystem, and each blockchain actually runs its own independent instance of consensus.

To launch a blockchain on Qmall, you just need to write a VM that defines your application; the consensus part is handled by the existing consensus engine implementations.
